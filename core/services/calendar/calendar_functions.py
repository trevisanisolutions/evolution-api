...
def create_appointments(args):
    ...
    for slot in slots:
        slot_args = {**args, **slot}
        result = _create_appointment(slot_args)
        results.append(result)

    return _get_final_result(results)
...
def check_availabilities(args):
    ...
    for slot in slots:
        slot_args = {**args, **slot}
        result = _check_availability(slot_args)
        results.append(result)
...
def cancel_appointments(args, is_admin=False):
    ...
    for slot in slots:
        slot_args = {**args, **slot}
        result = _cancel_appointment(slot_args, is_admin)
        results.append(result)
...
def reschedule_appointments(args, is_admin=False):
    ...
    for slot in slots:
        slot_args = {**args, **slot}
        result = _reschedule_appointment(slot_args, is_admin)
        results.append(result)
...
def _is_slot_available(start, end, events, timezone, user_phone, procedure_name, is_self_attendance,
                       procedure_capacity):
...
def _check_availability(args):
    ...
    try:
        ...
        def _generate_flexible_time_slots(start: str, end: str):
            time_format = "%H:%M"
        ...
        for schedule in schedules_today:
            all_slots.extend(_generate_flexible_time_slots(
                schedule["start_time"],
                schedule["end_time"]
            ))
        ...
        for slot_start, slot_end in all_slots:
            is_available, reason = _is_slot_available(
                slot_start,
                slot_end,
                events,
                TIMEZONE,
...
def _create_appointment(args):
    ...
    procedure_duration_minutes = args.get("procedure_duration_minutes", DEFAULT_APPOINTMENT_DURATION_SECONDS)
...
def _cancel_appointment(args, is_admin):
    professional_name = args.get("professional_name")
    procedure = args.get("procedure")
    user_phone = args.get("user_phone")
    professional_calendar_id = args.get("professional_calendar_id")
...
def _reschedule_appointment(args, is_admin):
    professional_name = args.get("professional_name")
    user_phone = args.get("user_phone")
    professional_calendar_id = args.get("professional_calendar_id")
